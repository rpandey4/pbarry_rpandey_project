...
def train(data_dict, word_embedding, classifier, model, dims, max_run):
    '''
    Train the LR/NN model
    Use GPU if available
    Only stop the model if the best (minimum) loss was max_run away from the current loss from the history
    Only store the best state if loss is decreased at every iteration
    '''
...
def test(data_dict, word_embedding, classifier, model, dims, m_name):
    '''
    test the test set based on trained model
    ...
    out_file_name = "output/%s_%s_%s_%s.txt" % ('test', m_name, word_embedding, now.strftime("%Y_%m_%d_%H_%M"))
...
def main():
    ...
    4. model to train (can be either lr or nn)
    '''
    ...
    for k in range(1, 100, 10):
        ...
        out_file_name = test(data_dict, word_embedding=args.word_embedding, classifier=model, model=w_model, dims=dims, m_name=args.model)
